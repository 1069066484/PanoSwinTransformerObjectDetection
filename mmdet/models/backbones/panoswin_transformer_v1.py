...
class WindowAttention(nn.Module):
    ...
    def _sphere_distance(self, v_coord=None):
        if v_coord is None:
            return 0
        # (v_coord.shape); exit() # torch.Size([135, 9])
        window_size = round(v_coord.shape[-1] ** 0.5)
        gap = torch.abs(v_coord[:,window_size] - v_coord[:,window_size-1])
        u_coord = torch.arange(window_size).to(v_coord.device).repeat(window_size)
        # print(v_coord.shape, u_coord.shape, gap.shape, )
        # exit()
        u_coord = u_coord[None,:] * gap[:,None]

        # torch.Size([36, 49]) torch.Size([36, 21]) torch.Size([36])
        # print(v_coord.shape, u_coord.shape, gap.shape, )
        # exit()
        uv = torch.stack([u_coord, v_coord], -1)
        sphere_distance = haversine22(uv, uv)

        B, H ,W = sphere_distance.shape

        sphere_position_alpha = self._make_bias(self.sphere_position_alpha_table, self.relative_position_index)
        sphere_position_beta = self._make_bias(self.sphere_position_beta_table, self.relative_position_index)
        ...
        # print("eeeeeee")
        # exit()
        # print(1111111, sphere_position.max(), sphere_position.min())
        return sphere_position
        # print(u_coord); exit() # torch.Size([135, 9]) torch.Size([135, 9])

    def _make_bias(self, table, index):
        return table[index.view(-1)].view(
            self.window_size[0] * self.window_size[1], self.window_size[0] * self.window_size[1], -1)


    def forward(self, x, mask=None, v_coord=None):
        """ Forward function.
        ...
        # print(self.relative_position_index.T)
        # exit()
        # print(self.relative_position_index)
        # exit()
        ...
        # print(self.relative_position_index.shape,  # torch.Size([9, 9])
        #       self.relative_position_bias_table.shape, # torch.Size([25, 3])
        #       relative_position_bias.shape # torch.Size([9, 9, 3])
        #       )

        # print(self.relative_position_index.view(-1))
        # print("xxxx", self.relative_position_bias_table.shape, self.relative_position_index.view(-1).shape, relative_position_bias.shape)
        # xxxx torch.Size([25, 3]) torch.Size([81]) torch.Size([9, 9, 3])

        relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous()  # nH, Wh*Ww, Wh*Ww
        # print("relative_position_bias", relative_position_bias.shape, attn.shape)
        # print(relative_position_bias)
        # print(v_coord[0], '\n', v_coord[1], '\n', v_coord[-11]); exit()
        # print(2222222, attn.shape, relative_position_bias.shape, v_coord.shape)
        # print(great_circle22())
        # 2222222 torch.Size([135, 3, 9, 9]) torch.Size([3, 9, 9]) torch.Size([135, 9])
        # print(relative_position_bias)
        sphere_distance = self._sphere_distance(v_coord)
...
class WindowTransition(nn.Module):
    ...
    def ew2ns(im):
        ...
        @param im: [b, w, h, c]
        """
        # assert im.shape[1] * 2 == im.shape[2], "Bad shape: {}".format(im.shape)
        # assert im.shape[2] % 2 == 0
        if im.shape[2] % 2:
    ...
    def forward(self, x, reverse=False):
        ...
        if self.win_trans_type == 0:
            ...
            if reverse:
                ...
                x = WindowTransition.ns2we(x)
                x = torch.roll(x, shifts=(-self.shift_size,), dims=(2,))
            else:
                x = torch.roll(x, shifts=(self.shift_size,), dims=(2,))
                x = WindowTransition.ew2ns(x)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix, v=None):
        ...
        if v is not None:
            v = v.view(B, H, W)
            v = torch.cat([v, v[:, -1:].repeat(1, pad_b, 1)], 1)
            v = torch.cat([v, v[:, :, -1:].repeat(1, 1, pad_r)], 2)
            x = torch.cat([x, v[..., None]], -1)

        # print("bbbbbbb", v.shape, x.shape)
        # cyclic shift
        # print(x.shape, 1234)
        ...
        # torch.Size([2, 126, 252, 96]) 7
        # print(shifted_x.shape, self.window_size) # torch.Size([2, 126, 252, 96]) 7
        # partition windows
        x_windows = window_partition(shifted_x, self.window_size)  # nW*B, window_size, window_size, C
        # print(222, x_windows.shape, self.window_size, C) # torch.Size([1296, 7, 7, 96]) 7 96

        x_windows = x_windows.view(-1, self.window_size * self.window_size, C + int(v is not None))  # nW*B, window_size*window_size, C
        # print(3333333, x_windows.shape, C) # torch.Size([1296, 49, 96]) 96

        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows[..., :-1], mask=attn_mask, v_coord=x_windows[..., -1])  # nW*B, window_size*window_size, C
        #print(x_windows.shape, attn_mask.shape if attn_mask is not None else None, attn_windows.shape)
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, H, W, stb_adj=None, v=None):
        ...
        # print(11111111, -self.shift_size)
        # print(img_mask[0, ..., 0])
        # print(img_mask.shape);exit()

        # print(11, img_mask.shape, self.window_size) # torch.Size([1, 42, 42, 1]) 7
        mask_windows = window_partition(img_mask, self.window_size)  # nW, window_size, window_size, 1
        # print(mask_windows.shape); exit()
        # print(mask_windows.shape) # torch.Size([36, 7, 7, 1])

        mask_windows = mask_windows.view(-1, self.window_size * self.window_size)
        # print(mask_windows.shape) # torch.Size([36, 49])

        attn_mask = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(2)
        # print(attn_mask.shape) # torch.Size([36, 49, 49])

        attn_mask = attn_mask.masked_fill(attn_mask != 0, float(-100.0)).masked_fill(attn_mask == 0, float(0.0))

        batch_size = x.shape[0]
        attn_mask = einops.rearrange(attn_mask, '(b d) p1 p2 -> b d p1 p2', b=batch_size)
        # print(batch_size, attn_mask.shape); exit()

        # print(mask_windows.shape, attn_mask.shape);exit() # torch.Size([648, 49]) torch.Size([648, 49, 49])
        # print(attn_mask.shape, len(self.blocks)) # torch.Size([36, 49, 49])
        # print(torch.unique(attn_mask, dim=0))
        # exit()
