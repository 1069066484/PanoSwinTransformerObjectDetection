...
from lzx.yolo.utils.general import is_ascii, xyxy2xywh, xywh2xyxy
from lzx.yolo.utils.metrics import fitness
...
def plot_one_box(box, im, color=(128, 128, 128), txt_color=(255, 255, 255), label=None, line_width=3, use_pil=False):
    ...
    if use_pil or not is_ascii(label):  # use PIL
        ...
        if label:
            font = ImageFont.truetype("/home/d3010/ZhixinLing/yolo5hw/fonts/simsun.ttc", size=max(round(max(im.size) / 40), 12))
...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=4):
    ...
    for i, img in enumerate(images):
        ...
        # print(img.shape, w, h) # ;exit() # (3, 384, 704)

        img = img.transpose(1, 2, 0)
        if scale_factor < 1:
            img = cv2.resize(img, (w, h))

        # cv2.imshow("img", img.astype(np.uint8));cv2.waitKeyEx()
