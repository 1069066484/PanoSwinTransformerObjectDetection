...
def _test_bb():
    ...
    patch = 10 # 500 // 7
    poses = ['center']
    # im = cv2.resize(im, (565, 432))
    # ------544-528-3     20   4    'center'     0.052,0.896
    # 99 554 (600, 1200, 3) [0.165, 0.9233333333333333]
    # im = im[250:, ]
    print(im.shape);#  exit()
    im = basketball_transition(im, patch_size=patch, poses=poses, ratio_v=(0.5, 1))
    for k in poses:
        im2 = im[k].copy()
    ...
    cv2.waitKey()
    exit()
    bb2 = basketball_transition_bb(im.shape, bb, patch_size=patch, poses=poses)
    uvf = basketball_uvmap_foreground(im[poses[0]].shape,  patch_size=patch)
